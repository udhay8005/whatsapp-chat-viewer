...
A single self‑contained Python script (`whatsapp_chat_viewer.py`) that reads a WhatsApp **"Export chat"** folder (the `.txt` transcript plus its media files) and generates a polished, **WhatsApp‑like** web page you can open by double‑clicking — no server, no internet, no installation of a browser extension, and **no CORS errors**.
...
Running the script adds the following to your export folder (both are **git‑ignored** here and never committed):
...
This tool is fully **offline**. Your chat content and media are processed locally and embedded into files on your own disk — nothing is uploaded anywhere. This repository intentionally contains **only the script** and documentation; no chat exports or media are included.
